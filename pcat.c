...
int getdict (void)
{
	...
	eof = &characters[0];
	inbuff[6] = 25;
	inleft = fread(&inbuff[0],1, BLKSIZE,infile);
	if (inleft < 0) {
		fprintf(stderr,".z: read error");
		return (0);
	}
	...
	maxlev = *inp++ & 0377;
	if (maxlev > 24) {
goof:		fprintf(stderr,".z: not in packed format");
		return (0);
	}
	for (i=1; i<=maxlev; i++)
		intnodes[i] = *inp++ & 0377;
...
int decode (void)
{
	...
	while (1) {
		if (inleft <= 0) {
			inleft = fread(inp = &inbuff[0],1, BLKSIZE, infile);
			if (inleft < 0) {
				fprintf(stderr,".z: read error");
				return (0);
			}
		...
		if (--inleft < 0) {
uggh:			fprintf(stderr,".z: unpacking error");
			return (0);
		}
		c = *inp++;
		bitsleft = 8;
...
wrerr:						fprintf(stderr,": write error");
						return (0);
					}
					origsize -= c;
...
getch() {
	if (inleft <= 0) {
		inleft = fread (infile, inp=inbuff, BLKSIZE);
		if (inleft < 0) {
			fprintf(stderr,".z: read error");
			longjmp (env, 1);
		}
